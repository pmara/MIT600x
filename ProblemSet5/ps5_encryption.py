...
def buildCoder(shift):
    ...
    for letter in string.ascii_lowercase:
        pos = (i + shift) % NUM_LETTERS
        coder[letter] = string.ascii_lowercase[pos]
        i += 1
    i = 0
    for letter in string.ascii_uppercase:
        pos = (i + shift) % NUM_LETTERS
        coder[letter] = string.ascii_uppercase[pos]
        i += 1
    return coder
...
def findBestShift(wordList, text):
    ...
    for shift in range(NUM_LETTERS):
        deciphered = applyShift(text, shift)
        ...
        # determine the shift with the max valid words
...
if __name__ == '__main__':
    ...
    #print applyShift(s, 0)
    #print applyShift(s, 26)