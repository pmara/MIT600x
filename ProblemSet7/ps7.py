...
class RectangularRoom(object):
    ...
    def __init__(self, width, height):
        ...
        self.width  = width
        self.height = height
        self.grid   = [ [ False for col in range(width) ] for row in range(height) ]
        self.clean  = 0
    ...
    def isPositionInRoom(self, pos):
        ...
        """
        if pos.getX() < 0 or self.width <= pos.getX():
            return False
        if pos.getY() < 0 or self.height <= pos.getY():
            return False
        return True
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        total_time += runTrial(num_robots, speed, width, height, min_coverage, robot_type)
